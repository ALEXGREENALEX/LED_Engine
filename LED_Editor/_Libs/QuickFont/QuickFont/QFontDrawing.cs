...
namespace QuickFont
{
    ...
    public class QFontDrawing
    {
        ...
        private static void InitialiseStaticState()
        {
            ...
            if (vert == 0 || frag == 0)
                throw new Exception(string.Format("Error creating shader name for {0}", vert == 0 ? (frag == 0 ? "vert and frag shaders" : "vert shader") : "frag shader"));
            ...
            {
                string vertInfo = GL.GetShaderInfoLog((int)vert);
                string fragInfo = GL.GetShaderInfoLog((int)frag);
            ...
            if (progLinkStatus == 0)
            {
                string programInfoLog = GL.GetProgramInfoLog((int)prog);
    ...
    {
        public ShaderVariables(uint shaderProgram, int mvpUniformLocation, int textureCoordAttribLocation, int positionCoordAttribLocation, int samplerLocation, int colorCoordAttribLocation)
        {
