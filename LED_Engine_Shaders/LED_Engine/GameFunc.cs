...
namespace LED_Engine
{
    partial class Game
    {
        ...
        static void LoadResourcesFromMap_FAKE()
        {
            ...
            {
                Mesh DebugPointLight = Mesh.LoadFromFile(Engine.CombinePaths(Settings.Paths.EngineMeshes, "PointLight.obj"));
            ...
            plain.Material = Materials.Load("Light");
            plain.Position.Y = -0.5f;
            Objects.Add(plain);

            Mesh cube = Mesh.MakeCube();
            cube.Material = Materials.Load("Light");
            cube.Position.X = -2f;
            Objects.Add(cube);

            Mesh Pipe_X_1 = Mesh.LoadFromFile(Engine.CombinePaths(Settings.Paths.Meshes, "Pipe_X.obj"));
            ...
            Mesh obj_Keypad = Mesh.LoadFromFile(Engine.CombinePaths(Settings.Paths.Meshes, "Keypad.obj"));
            ...
            int a = 33;
            Prefab prefab1 = new Prefab(new Mesh[a * a]);
            for (int i1 = 0; i1 < a; i1++)
                for (int i2 = 0; i2 < a; i2++)
                {
                    prefab1.Objects[i1 * a + i2] = new Mesh(obj_Keypad);
                    prefab1.Objects[i1 * a + i2].Material = Materials.Load("Light"); //ReliefParallaxTest //Keypad //Keypad
            ...
            Mesh obj_Teapot = Mesh.LoadFromFile(Engine.CombinePaths(Settings.Paths.EngineMeshes, "Teapot.obj"));
